...
void Shutdown()
{
    ...
    RenameThread("IntuitionCoin-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: IntuitionCoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: IntuitionCoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeIntuitionCoinamount=<n> " + _("Keep N IntuitionCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. IntuitionCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. IntuitionCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("IntuitionCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "IntuitionCoin server starting\n"); 
...
#ifdef ENABLE_WALLET
    if (mapArgs.count("-reservebalance")) // IntuitionCoinCoin: reserve balance amount
    {
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of IntuitionCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart IntuitionCoin to complete") << "\n";
    ...
    nAnonymizeIntuitionCoinAmount = GetArg("-anonymizeIntuitionCoinamount", 0);
    if(nAnonymizeIntuitionCoinAmount > 999999) nAnonymizeIntuitionCoinAmount = 999999;
    if(nAnonymizeIntuitionCoinAmount < 2) nAnonymizeIntuitionCoinAmount = 2;
    ...
    LogPrintf("Anonymize IntuitionCoin Amount %d\n", nAnonymizeIntuitionCoinAmount);
