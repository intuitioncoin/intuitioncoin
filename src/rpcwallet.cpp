...
CScript _createmultisig(const Array& params)
{
    ...
    for (unsigned int i = 0; i < keys.size(); i++)
    {
        ...
        CIntuitionCoinCoinAddress address(ks);
...
Value createmultisig(const Array& params, bool fHelp)
{
    ...
    CIntuitionCoinCoinAddress address(innerID);
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nReturns a new IntuitionCoin address for receiving payments.\n"
            "If 'account' is specified (recommended), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) The account name for the address to be linked to. if not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
            "\nResult:\n"
            "\"IntuitionCoinaddress\"    (string) The new IntuitionCoin address\n"
            "\nExamples:\n"
    ...
    return CIntuitionCoinCoinAddress(keyID).ToString();
}


CIntuitionCoinCoinAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CIntuitionCoinCoinAddress(account.vchPubKey.GetID());
}
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturns the current IntuitionCoin address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"IntuitionCoinaddress\"   (string) The account IntuitionCoin address\n"
            "\nExamples:\n"
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount \"IntuitionCoinaddress\" \"account\"\n"
            "\nSets the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"IntuitionCoinaddress\"  (string, required) The IntuitionCoin address to be associated with an account.\n"
    ...
    CIntuitionCoinCoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid IntuitionCoin address");
...
Value getaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccount \"IntuitionCoinaddress\"\n"
            "\nReturns the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"IntuitionCoinaddress\"  (string, required) The IntuitionCoin address for account lookup.\n"
            "\nResult:\n"
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"IntuitionCoinaddress\"  (string) a IntuitionCoin address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
    ...
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CIntuitionCoinCoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CIntuitionCoinCoinAddress& address = item.first;
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress \"IntuitionCoinaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSent an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
           "1. \"IntuitionCoinaddress\"  (string, required) The IntuitionCoin address to send to.\n"
            "2. \"amount\"      (numeric, required) The amount in INIT to send. eg 0.1\n"
...
Value listaddressgroupings(const Array& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"IntuitionCoinaddress\",     (string) The IntuitionCoin address\n"
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        {
            Array addressInfo;
            addressInfo.push_back(CIntuitionCoinCoinAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                LOCK(pwalletMain->cs_wallet);
                if (pwalletMain->mapAddressBook.find(CIntuitionCoinCoinAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CIntuitionCoinCoinAddress(address).Get())->second);
            }
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage \"IntuitionCoinaddress\" \"message\"\n"
            "\nSign a message with the private key of an address"
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"IntuitionCoinaddress\"  (string, required) The IntuitionCoin address to use for the private key.\n"
    ...
    CIntuitionCoinCoinAddress addr(strAddress);
    if (!addr.IsValid())
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress \"IntuitionCoinaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given IntuitionCoinaddress in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
            "1. \"IntuitionCoinaddress\"  (string, required) The IntuitionCoin address for transactions.\n"
            "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "\nResult:\n"
            "amount   (numeric) The total amount in INIT received at this address.\n"
            "\nExamples:\n"
    ...
    // Bitcoin address
    CIntuitionCoinCoinAddress address = CIntuitionCoinCoinAddress(params[0].get_str());
    CScript scriptPubKey;
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid IntuitionCoin address");
...
Value getreceivedbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "2. minconf          (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "\nResult:\n"
            "amount              (numeric) The total amount in INIT received for this account.\n"
            "\nExamples:\n"
...
Value movecmd(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw runtime_error(
            ...
            "\nExamples:\n"
            "\nMove 0.01 INIT from the default account to the account named tabby\n"
            + HelpExampleCli("move", "\"\" \"tabby\" 0.01") +
            "\nMove 0.01 INIT timotei to akiko with a comment and funds have 10 confirmations\n"
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"toIntuitionCoinaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nSent an amount from an account to a IntuitionCoin address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"toIntuitionCoinaddress\"  (string, required) The IntuitionCoin address to send funds to.\n"
            "3. amount                (numeric, required) The amount in INIT. (transaction fee is added on top).\n"
            ...
            "\"transactionid\"        (string) The transaction id.\n"
            "\nExamples:\n"
            "\nSend 0.01 INIT from the default account to the address, must have at least 1 confirmation\n"
    ...
    string strAccount = AccountFromValue(params[0]);
    CIntuitionCoinCoinAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid IntuitionCoin address");
    CAmount nAmount = AmountFromValue(params[2]);
...
Value sendmany(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The IntuitionCoin address is the key, the numeric amount in INIT is the value\n"
            "      ,...\n"
            "    }\n"
    ...
    set<CIntuitionCoinCoinAddress> setAddress;
    vector<pair<CScript, int64_t> > vecSend;

    int64_t totalAmount = 0;
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        CIntuitionCoinCoinAddress address(s.name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid IntuitionCoin address: ")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "Each key is a IntuitionCoin address or hex-encoded public key.\n"
            "If 'account' is specified, assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of IntuitionCoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) IntuitionCoin address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"IntuitionCoinaddress\"  (string) A IntuitionCoin address associated with the keys.\n"
    ...
    return CIntuitionCoinCoinAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<CIntuitionCoinCoinAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CIntuitionCoinCoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CIntuitionCoinCoinAddress, tallyitem>::iterator it = mapTally.find(address);
...
Value listreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in INIT received by the address\n"
...
{
    CIntuitionCoinCoinAddress addr;
    if (addr.Set(dest))
...
Value listtransactions(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"IntuitionCoinaddress\",    (string) The IntuitionCoin address of the transaction. Not present for \n"
            "                                                move transactions (category = move).\n"
            "    \"category\":\"send|receive|move\", (string) The transaction category. 'move' is a local (off blockchain)\n"
            "                                                transaction between accounts, and not associated with an address,\n"
            "                                                transaction id or block. 'send' and 'receive' transactions are \n"
            "                                                associated with an address, transaction id and block details\n"
            "    \"amount\": x.xxx,          (numeric) The amount in INIT. This is negative for the 'send' category, and for the\n"
...
Value listsinceblock(const Array& params, bool fHelp)
{
    if (fHelp)
       throw runtime_error(
            ...
            "    \"address\":\"IntuitionCoinaddress\",    (string) The IntuitionCoin address of the transaction. Not present for move transactions (category = move).\n"
            "    \"category\":\"send|receive\",     (string) The transaction category. 'send' has negative amounts, 'receive' has positive amounts.\n"
            "    \"amount\": x.xxx,          (numeric) The amount in INIT. This is negative for the 'send' category, and for the 'move' category for moves \n"
...
Value gettransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "      \"address\" : \"IntuitionCoinaddress\",   (string) The IntuitionCoin address involved in the transaction\n"
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending INIT\n"
            "\nArguments:\n"
    ...
    // IntuitionCoinCoin: if user OS account compromised prevent trivial sendmoney commands
    if (params.size() > 2)
...
Value encryptwallet(const Array& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending IntuitionCoin\n"
            + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n"
            + HelpExampleCli("signmessage", "\"IntuitionCoinaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; IntuitionCoin server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}


// IntuitionCoinCoin: reserve balance from being staked for network protection
...
// IntuitionCoinCoin: check wallet integrity
...
// IntuitionCoinCoin: repair wallet
...
// IntuitionCoinCoin: resend unconfirmed wallet transactions
...
// IntuitionCoinCoin: make a public-private key pair
...
Value settxfee(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in INIT/kB rounded to the nearest 0.00000001\n"
            "\nResult\n"
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new IntuitionCoin stealth address for receiving payments anonymously.  ");
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    {
        result.push_back(Pair("result", "Invalid IntuitionCoin stealth address."));
        return result;
    };
