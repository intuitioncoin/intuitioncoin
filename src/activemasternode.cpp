...
//
// Bootup the masternode, look for a 500 INIT input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in IntuitionCoin.conf */
...
{
    CIntuitionCoinCoinAddress address(collateralAddress);
