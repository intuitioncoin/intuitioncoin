...
int nAnonymizeIntuitionCoinAmount = 1000;
...
#else
    const char* pszModule = "IntuitionCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\IntuitionCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\IntuitionCoin
    // Mac: ~/Library/Application Support/IntuitionCoin
    // Unix: ~/.IntuitionCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "IntuitionCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "IntuitionCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "IntuitionCoind.pid"));
