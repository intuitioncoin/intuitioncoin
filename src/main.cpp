...
const string strMessageMagic = "IntuitionCoin Signed Message:\n";
                        ...
                        bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    // IntuitionCoinCoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// IntuitionCoinCoin: find block wanted by given orphan block
...
int64_t GetPosRewardTieredMN(const CBlockIndex* pindexPrev)
{
    ...
    if (nHeight > 1 && nHeight <= 2001) 
        return POS_REWARD_TIERED_MN_LVL1;
    if (nHeight > 2001 && nHeight <= 21001) 
        return POS_REWARD_TIERED_MN_LVL2;
	if (nHeight > 21001 && nHeight <= 63001) 
        return POS_REWARD_TIERED_MN_LVL3;
	if (nHeight > 63001 && nHeight <= 189001) 
        return POS_REWARD_TIERED_MN_LVL4;
	if (nHeight > 189001 && nHeight <= 350000) 
...
// intuitioncoinCoin: find last block index up to pindex
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // IntuitionCoinCoin: target change every block
    // IntuitionCoinCoin: retarget with exponential moving toward target spacing
    CBigNum bnNew;
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags, bool fValidateSig)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // IntuitionCoinCoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // IntuitionCoinCoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // IntuitionCoinCoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // IntuitionCoinCoin: track money supply and mint amount info
#ifndef LOWMEM
...
// IntuitionCoinCoin: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // IntuitionCoinCoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // IntuitionCoinCoin: compute stake entropy bit for stake modifier
    ...
    // IntuitionCoinCoin: compute stake modifier
        ...
        if(MasternodePayments)
        {
            ...
            if(IsProofOfStake() && pindex != NULL){
                if(pindex->GetBlockHash() == hashPrevBlock){
                    ...
                    if(!checkoff){
                        ...
                        if(!winningNode){
                            ...
                            if(fDebug) {
                                ...
                                CIntuitionCoinCoinAddress address23(addressnned1);
                                CIntuitionCoinCoinAddress address24(addressnned2);
                            ...
                            for (unsigned int i = 0; i < vtx[1].vout.size(); i++) {
                                ...
                                if(fDebug) { CTxDestination addressfound;
                                    ...
                                    CIntuitionCoinCoinAddress address25(addressfound);
                    ...
                    CIntuitionCoinCoinAddress address2(address1);
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // IntuitionCoinCoin: check proof-of-stake
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // IntuitionCoinCoin: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// IntuitionCoinCoin: attempt to generate suitable proof-of-stake
...
{
    RenameThread("IntuitionCoin-loadblk");
