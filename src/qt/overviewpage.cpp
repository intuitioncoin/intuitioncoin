...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeIntuitionCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeIntuitionCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeIntuitionCoinAmount = strAnonymizeIntuitionCoinAmount.remove(strAnonymizeIntuitionCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeIntuitionCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeIntuitionCoinAmount*COIN) nMaxToAnonymize = nAnonymizeIntuitionCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeIntuitionCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeIntuitionCoinAmount));
        strAnonymizeIntuitionCoinAmount = strAnonymizeIntuitionCoinAmount.remove(strAnonymizeIntuitionCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeIntuitionCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeIntuitionCoinAmount == 0){
