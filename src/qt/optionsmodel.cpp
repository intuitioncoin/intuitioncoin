...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeIntuitionCoinAmount"))
        settings.setValue("nAnonymizeIntuitionCoinAmount", 1000);
    nAnonymizeIntuitionCoinAmount = settings.value("nAnonymizeIntuitionCoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeIntuitionCoinAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeIntuitionCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeIntuitionCoinAmount:
            return QVariant(nAnonymizeIntuitionCoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeIntuitionCoinAmount:
            nAnonymizeIntuitionCoinAmount = value.toInt();
            settings.setValue("nAnonymizeIntuitionCoinAmount", nAnonymizeIntuitionCoinAmount);
            emit AnonymizeIntuitionCoinAmountChanged(nAnonymizeIntuitionCoinAmount);
            break;
        default:
            break;
        }
