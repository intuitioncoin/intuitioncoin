...
{
    // IntuitionCoinCoin: check prefix
    if(uri.scheme() != QString("IntuitionCoin"))
        return false;
...
{
    // Convert IntuitionCoin:// to IntuitionCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("IntuitionCoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "IntuitionCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "IntuitionCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for IntuitionCoin.lnk
...
{
    return GetAutostartDir() / "IntuitionCoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a IntuitionCoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=IntuitionCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the IntuitionCoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add IntuitionCoin app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("IntuitionCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  IntuitionCoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("IntuitionCoin-Qt"));
